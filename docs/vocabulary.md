...
* **Administrator**: A person or automation process (i.e., CI/CD pipeline) that creates, destoys, updates or otherwise maintains a Compliant Kubernetes environment.
* **Control**: "Points" in an organization that need a clear policy in order to comply with regulation.
* **Environment**: A Compliant Kubernetes environment is an installation administared as a single unit. Implementation-wise, a Compliant Kubernetes environment generally consists of a workload cluster and a service cluster.
...
* **User**: A person or an automation process (i.e., CI/CD pipeline) that interacts with Compliant Kubernetes for the purpose of running and monitoring an application hosted by a Compliant Kubernetes environment.
